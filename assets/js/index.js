...
const makeTen = () => {
	const str = 'Hello';
	const resultTen = str[2];
	console.log(`${resultTen}`);
};
//Добавьте слушатель события
//Не работает слушатель
...
const makeSixteen = () => {
	const str = 'JavaScript';
	resultSixteen.innerHTML = str.substring(3, 10)
};
...
const makeSeventeen = () => {
	const str = 'JavaScript';
	resultSeventeen.innerHTML = str.substring(3, 6);
};
...
const makeNineteen = () => {
	const str = 'Hello, world!';
	resultNineteen.innerHTML = str.substring(7, str.length);
};
...
const makeTwentySix = () => {
	const resTwS = Math.random();
	resultTwentySix.innerHTML = resTwS.toFixed(4);
};
